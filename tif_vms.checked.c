...
{
	_TIFF_stat_s sb;
	return (toff_t) (_TIFF_fstat_f((int) fd, ((struct stat *)&sb)) < 0 ? 0 : sb.st_size);
}
...
#else /* !HAVE_MMAP */
static int _tiffMapProc(thandle_t fd, tdata_t *pbase, _Ptr<toff_t> psize)
{
	return (0);
}
...
static void vmsWarningHandler(const char *module, const char *fmt, va_list ap)
{
	if (module != NULL)
...
NOSHARE _Ptr<void (const char *, const char *, struct __va_list_tag *)> _TIFFwarningHandler =  vmsWarningHandler
...
NOSHARE _Ptr<void (const char *, const char *, struct __va_list_tag *)> _TIFFerrorHandler =  vmsErrorHandler
#if defined(VAX) && defined(__GNUC__)
