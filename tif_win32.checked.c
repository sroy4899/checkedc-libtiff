...
TIFF * TIFFFdOpen(int ifd, const char *name, const char *mode)
{
	...
	tif = TIFFClientOpen(name, mode, ((thandle_t )thandle_from_int(ifd)),
 ...
 */
TIFF * TIFFOpenW(_Ptr<const int> name, const char *mode)
{
	...
	if (mbsize > 0) {
		...
		if (!mbname) {
			...
			((const char *)"Can't allocate space for filename conversion buffer"));
			return ((TIFF*)0);
		}
	...
	tif = TIFFFdOpen(thandle_to_int(fd),
			 ((const char *)(mbname != NULL) ? mbname : "<unknown>"), mode);
	if(!tif)
		CloseHandle(fd);
...
_Ptr<void (const char *, const char *, struct __va_list_tag *)> _TIFFerrorHandler =  Win32ErrorHandler;
