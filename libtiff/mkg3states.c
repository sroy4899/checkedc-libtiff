...
void WriteTable(FILE *fd, const TIFFFaxTabEnt *T, int Size, const char *name : itype(_Ptr<const char>))
{
    int i;
    char* sep;

    fprintf(fd, ((const char *)"%s %s TIFFFaxTabEnt %s[%d] = {"),
	storage_class, const_class, name, Size);
    if (packoutput) {
	sep = "\n";
	for (i = 0; i < Size; i++) {
	    fprintf(fd, ((const char *)"%s%s%d,%d,%d%s"),
	...
	sep = "\n ";
	for (i = 0; i < Size; i++) {
	    fprintf(fd, ((const char *)"%s%s%3d,%3d,%4d%s"),
		sep, prebrace, T->State, T->Width, (int) T->Param, postbrace);
	    if (((i+1) % 6) == 0)
		    sep = ",\n ";
	    else
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, ((const char *)"c:s:bp"))) != -1)
	switch (c) {
	case 'c':
	    const_class = optarg;
	    break;
	...
	case '?':
	    fprintf(stderr,
		((const char *)"usage: %s [-c const] [-s storage] [-p] [-b] file\n"),
		argv[0]);
	    return (-1);
	}
    outputfile = optind < argc ? argv[optind] : "g3states.h";
    fd = fopen(outputfile, ((const char *)"w"));
    if (fd == NULL) {
	fprintf(stderr, ((const char *)"%s: %s: Cannot create output file.\n"),
	    argv[0], outputfile);
	return (-2);
    }
    ...
    fprintf(fd, ((const char *)"/* WARNING, this file was automatically generated by the\n"));
    fprintf(fd, ((const char *)"    mkg3states program */\n"));
    fprintf(fd, ((const char *)"#include \"tiff.h\"\n"));
    fprintf(fd, ((const char *)"#include \"tif_fax3.h\"\n"));
